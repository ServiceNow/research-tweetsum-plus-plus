...
class Lead(Backbone):
    def train(self):
        ...
        self.output_dir = "/mnt/colab_public/gebelangsn/factsumm/lead"
    ...
    def predict(self):
        ...
        for i, dialog in enumerate(self.test_dataset["dialog_formatted"]):
            ...
            # select first two sentences from the dialog where is_agent is True and first two sentences where is_agent is False
            extsum_sentences = [' '.join(dialog[i]['sentences']) for i in range(len(dialog)) if dialog[i]['is_agent'] is True][:2] + [' '.join(dialog[i]['sentences']) for i in range(len(dialog)) if dialog[i]['is_agent'] is False][:2]
