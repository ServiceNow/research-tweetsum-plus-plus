...
def init_dataset():
    data = pd.read_excel(
        "/Users/georges.belangeralba/Documents/udem/thesis/project2024/january/20240129_TweetSum_Abhay.xlsx"
    )
    data = data.head(34)
...
def parse_summary_improved(summary):
    # Check if the summary is a string
    if not isinstance(summary, str):
        return []  # Return an empty list or other default value for non-string inputs
...
if __name__ == "__main__":
    ...
    abhay_df = init_dataset()

    processor = TweetSumProcessor("twcs/twcs.csv")
    with open("tweet_sum_data_files/final_test_tweetsum.jsonl") as f:
        dialog_with_summaries = processor.get_dialog_with_summaries(f.readlines())
        for i, dialog_with_summary in dialog_with_summaries[25:]:
            json_format = dialog_with_summary.get_json()
            string_format = str(dialog_with_summary)

            # identify which dialogs are in the abhay dataset abhay_df['dialog_formatted']
            ...
            # print(abhay_df['dialog_formatted'])
            # print(dialog_with_summary.get_dialog().dialog_id)
